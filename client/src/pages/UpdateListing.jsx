...
const UpdateListing = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (listingImage.length > 6)
        ...
        if (listingImage.length < 2)
          return toast.error("images must minimum length of two");
      if (inputs.regularPrice < inputs.discountPrice)
        return toast.error("Discount price must be lower than regular price");
      setLoading(true);
      const res = await fetch(`${baseUrl}/listing/update-listing/${id}`, {
        method: "PUT",
        credentials: "include",
        headers: {
  ...
  return (
    <div className="mt-20 max-w-6xl mx-auto">
      <div className="ml-3">
        {" "}
        <Breadcrumb title="Update" subtitle="Profile" />
      </div>
      <Helmet>
        <title>Estate Mart - Update Listing</title>
      </Helmet>
      <div className=" flex flex-col  px-5 justify-center h-fit my-5">
        <h3 className="md:text-3xl text-2xl font-medium text-gray-800 py-5 text-center">
          Update Listing
        </h3>
        <form
        ...
        >
          <div className="w-full flex flex-col gap-5">
            ...
            <input
              ...
              required
            />
            <div className="flex items-center flex-wrap  gap-x-5 gap-y-0">
              ...
              <div className="flex items-center flex-nowrap mb-4">
                ...
                <label
                  htmlFor="parking"
                  className="ms-2 text-sm font-medium text-gray-600 whitespace-nowrap"
                >
                  Parking Spot
                </label>
              </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className=" bg-blue rounded-md py-2 px-3 w-full text-white hover:opacity-90 transition-all duration-150 ease-in-out disabled:opacity-90 flex justify-center items-center"
            >
              {loading ? (
                <ThreeDots
                  visible={true}
